...
(defun %make-rand-by-grid-config (n lengths &key (ratio 0.1))
  "Make `n' particles config randomly distributed on geometry `lengths'.

The particles are generated by first init with a grid config
by `%make-grid-config' then apply a random offset with size of `ratio'.
"
  (let ((config (%make-grid-config n lengths))
        (dim    (length lengths)))
    (piter-i* ((i n))
      (setf (aref config i)
            (map 'vector (lambda (v l) (mod v l))
                 (vec-plus-vec (aref config i)
                               (num-times-vec (random ratio) (random-vector dim)))
                 lengths)))
    config))
...
(defmethod initialize-instance :after
  ...
  (with-slots (lengths config) system
    ...
    (let ((n  (system-size system))
          (rc (system-cutoff system)))
      (ecase scheme
        ((:grid :grided)
         (setf config (%make-grid-config n lengths)))
        ((:rand :random)
         (setf config
               (if (periodic? system)
                   (%make-period-rand-config n lengths :min-distance rc)
                   (%make-rand-config n lengths :min-distance rc))))
        ((:rand-by-grid :grided-rand :grided-random)
         (setf config (%make-rand-by-grid-config n lengths :ratio (/ rc 10))))
